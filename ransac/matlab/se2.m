...
for i = 1:iter
    ...
    F = [eye(3), dt*Jr(dt*v);
        zeros(3,3), adjoint(dt*v)*Jr(dt*v) - eye(3)];
...
g2 = g1*expm(wedge(U));
U2 = Adjoint(expm(-wedge(U)))*u;
g2d1 = g1*expm(wedge(d1*dt))*expm(wedge(Adjoint(expm(-wedge(d1*dt)))*U));
g2d2 = g1*expm(wedge(d2*dt))*expm(wedge(Adjoint(expm(-wedge(d2*dt)))*U));
g2d3 = g1*expm(wedge(d3*dt))*expm(wedge(Adjoint(expm(-wedge(d3*dt)))*U));

U2d1 = Adjoint(expm(-wedge(d1*dt)))*Adjoint(expm(-wedge(U)))*u
U2d2 = Adjoint(expm(-wedge(d2*dt)))*Adjoint(expm(-wedge(U)))*u
U2d3 = Adjoint(expm(-wedge(d3*dt)))*Adjoint(expm(-wedge(U)))*u

% U2d1 = Adjoint(expm(-wedge(Adjoint(expm(-wedge(d1*dt)))*U)))*Adjoint(expm(-wedge(d1*dt)))*u;
% U2d2 = Adjoint(expm(-wedge(Adjoint(expm(-wedge(d2*dt)))*U)))*Adjoint(expm(-wedge(d2*dt)))*u;
% U2d3 = Adjoint(expm(-wedge(Adjoint(expm(-wedge(d3*dt)))*U)))*Adjoint(expm(-wedge(d3*dt)))*u;

[ev1, eu1] = ominus(g2,U2,g2d1,U2d1);
ev1 = ev1/dt;
eu1 = eu1/dt;

[ev2, eu2] = ominus(g2,U2,g2d2,U2d2);
ev2 = ev2/dt;
eu2 = eu2/dt;

[ev3, eu3] = ominus(g2,U2,g2d3,U2d3);
ev3 = ev3/dt;
eu3 = eu3/dt;
N = [ev1, ev2, ev3]
M = [eu1,eu2,eu3]
